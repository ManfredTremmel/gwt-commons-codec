...
public class QuotedPrintableCodec implements BinaryEncoder, BinaryDecoder, StringEncoder, StringDecoder {
     ...
     * @return The number of bytes written to the <code>buffer</code>
     */
    private static final int encodeQuotedPrintable(final int b, final byte[] buffer, final int bufPos) {
    	buffer[bufPos] = (byte) ESCAPE_CHAR;
        final char hex1 = Character.toUpperCase(Character.forDigit((b >> 4) & 0xF, 16));
        final char hex2 = Character.toUpperCase(Character.forDigit(b & 0xF, 16));
        buffer[bufPos + 1] = (byte) hex1;
        buffer[bufPos + 2] = (byte) hex2;
        return 3;
    }
     ...
     */
    private static int encodeByte(final int b, final boolean encode,
                                  final byte[] buffer, final int bufPos) {
        if (encode) {
            return encodeQuotedPrintable(b, buffer, bufPos);
        } else {
    ...
    public static final byte[] encodeQuotedPrintable(BitSet printable, final byte[] bytes, boolean strict) {
        ...
        final byte[] buffer = new byte[bytes.length * 6];
        int bufPos = 0;
        ...
        if (strict) {
            ...
            for (int i = 0; i < bytes.length - 3; i++) {
                ...
                if (pos < SAFE_LENGTH) {
                	...
                	bufPos += encodeByte(b, !printable.get(b) || isWhitespace(b), buffer, bufPos);

                    // rule #5: soft line break
                    buffer[bufPos++] = (byte) ESCAPE_CHAR;
                    buffer[bufPos++] = (byte) CR;
                    buffer[bufPos++] = (byte) LF;
                    pos = 1;
                }
            }
            ...
            final int addedBytes = encodeByte(b, encode, buffer, bufPos);
            pos += addedBytes;
            bufPos += addedBytes;
            ...
            for (int i = bytes.length - 2; i < bytes.length; i++) {
                ...
                bufPos += encodeByte(b, encode, buffer, bufPos);
            }
        } else {
            for (final byte c : bytes) {
                ...
                if (printable.get(b)) {
                	...
                	bufPos += encodeQuotedPrintable(b, buffer, bufPos);
                }
            }
        }
    ...
    public String decode(final String str, final Charset charset) throws DecoderException {
        ...
        return StringUtils.newString(this.decode(StringUtils.getBytesUsAscii(str)), charset.name());
    }
